...
public class GerenciadorInteracao {
    ...
    private static void usarChave(Jogador jogador, Sala salaAtual) {
        // 0. Chave da Porta Principal (Usada na Sala de Estar para abrir a Sacada)
        if (jogador.possuiItem("Chave da Porta Principal") || jogador.possuiItem("Chave de Saída")) {
            Sala sacada = salaAtual.pegarSaida("sul"); // Ou "saida"
            if ((salaAtual.pegarNome().equalsIgnoreCase("Sala de Estar")
                    || salaAtual.pegarNome().equalsIgnoreCase("Sala"))
                    && sacada != null && sacada.estaTrancada()) {
        ...
        if (jogador.possuiItem("Chave do Porão")) {
            ...
            if (salaAtual.pegarNome().equalsIgnoreCase("Sala de Jantar") && escadaPorao != null
                    && escadaPorao.estaTrancada()) {
                escadaPorao.destrancarCom("Chave do Porão");
                Configuracao.digitar(
                        "Insero a Chave do Porão na fechadura pesada. A porta para o porão se destranca com um estalo!");
                return;
            }
        }
        ...
        if (jogador.possuiItem("Chave do Quarto do Morador")) {
            ...
            if (salaAtual.pegarNome().equalsIgnoreCase("Hall") && quartoMorador != null
                    && quartoMorador.estaTrancada()) {
        ...
        if (jogador.possuiItem("Chave do Guarda-Roupa") && salaAtual.pegarNome().equalsIgnoreCase("Quarto Morador")) {
            ...
            if (guardaRoupa != null) {
                if (guardaRoupa.estaTrancado()) {
                    ...
                    Configuracao.digitar(
                            "Destranco o Guarda-Roupa Grande com a chave simples. As portas se abrem devagar...");
        ...
        // Caso o jogador tente usar uma chave na sala errada ou sem porta trancada por
        // perto
    ...
    private static void usarPilhas(Jogador jogador) {
        if (jogador.possuiItem("Gravador de Áudio") && jogador.possuiItem("Pilhas")) {
            ...
            Configuracao.digitar(
                    "Eu encaixo as duas pilhas de 1,5V no compartimento do Gravador de Áudio. A luz vermelha pisca!");
        } else {
    ...
    private static void usarFita(Jogador jogador, String entradaAlvo) {
        ...
        if ((entradaAlvo.contains("1") || entradaAlvo.contains("porque?") || entradaAlvo.contains("fita"))
                && jogador.possuiItem("Fita Cassete #01")) {
        ...
        } else if ((entradaAlvo.contains("2") || entradaAlvo.contains("acusações") || entradaAlvo.contains("fita"))
                && jogador.possuiItem("Fita Cassete #02")) {
        ...
        } else if ((entradaAlvo.contains("3") || entradaAlvo.contains("pistas") || entradaAlvo.contains("fita"))
                && jogador.possuiItem("Fita Cassete #03")) {
        ...
        } else if ((entradaAlvo.contains("4") || entradaAlvo.contains("decisão") || entradaAlvo.contains("fita"))
                && jogador.possuiItem("Fita Cassete #04")) {
    ...
    public static boolean isGeradorLigado() {
        return geradorLigado;
    }
}
